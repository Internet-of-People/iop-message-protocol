...
###### Step 27:
  ...
  * `UpdateActivityRequest.activity.longitude := 2`
  * `UpdateActivityRequest.activity.precision := 0`
  * `UpdateActivityRequest.activity.startTime` is set to current UTC time stamp + 2 hours
  * `UpdateActivityRequest.activity.expirationTime` is set to current UTC time stamp + 25 hours
  * `UpdateActivityRequest.activity.extraData := ""`
  * `UpdateActivityRequest.noPropagation := false`
  * `UpdateActivityRequest.ignoreServerIds := []`
  * `ConversationRequest.signature` is a valid signature of `UpdateActivityRequest.activity` part of the message using the test's identity private key
...
###### Step 32:
  ...
  * `UpdateActivityRequest.activity.profileServerContact.ipAddress := 127.0.0.1`
  * `UpdateActivityRequest.activity.profileServerContact.primaryPort := 16987`
  * `UpdateActivityRequest.activity.type := "new"`
  * `UpdateActivityRequest.activity.latitude := 1`
  * `UpdateActivityRequest.activity.longitude := 2`
  * `UpdateActivityRequest.activity.precision := 0`
  * `UpdateActivityRequest.activity.startTime` is set to current UTC time stamp
...
#### PX03012 - Delete Activity - Unauthorized
...
The test sends *DeleteActivityRequest* to the proximity server without starting the conversation first.
Then it starts the conversation and sends *DeleteActivityRequest* without verifying its identity first.
...
The test establishes a TLS connection to the client port of the proximity server and sends *DeleteActivityRequest*:

  * `Message.id := 1`
  * `DeleteActivityRequest.id := 1`
...
and reads the response. Then it sends *DeleteActivityRequest*:
...
#### PX03013 - Delete Activity - Bad Role
...
The test sends *DeleteActivityRequest* to the proximity server's primary and neighbor ports, but this request requires client port.
...
Then it sends *DeleteActivityRequest*:
...
The test creates and deletes and activity and checks that the activity was deleted.
Then it tries to delete non-existing activities.
...
The test sends *GetActivityInformationRequest*:

  * `Message.id := 4`
  * `SingleRequest.version := [1,0,0]`
  * `GetActivityInformationRequest.id := 1`
  * `GetActivityInformationRequest.ownerNetworkId` is set to test identity network identifier
...
The test sends *DeleteActivityRequest*:
...
Server replies with *GetActivityInformationResponse*:
  
  * `Message.id == 4`
  * `Response.status == STATUS_OK`
  * `GetActivityInformationResponse.info.isPrimary == true` 
  * `GetActivityInformationResponse.info.signedActivity.activity` describes the activity
  * `GetActivityInformationResponse.info.signedActivity.signature` is a valid signature of `GetActivityInformationResponse.info.signedActivity.activity`


###### Step 3:

Server replies with *DeleteActivityResponse*:
...
#### PX03015 - Get Activity Information
...
The test creates two activities. One on the tested proximity server, one on a proximity server in the neighborhood.
It then checks that information about both activities can be obtained through the tested proximity server.
...
The test simulates an additional proximity server with location at [10, 20] and network ID `$Server2Id` (Server2), 
running its primary port service on $BasePort2 and neighor port on $BasePort2+1.

The test simulates a location based network server with location at [0, 0] let's the target proximity server to perform the LOC initialization 
with the simulated location server. 

Then test's location server sends a notification message to the proximity server with the information that the simulated proximity server
is in its neighborhood.

The simulated proximity server then waits for *StartNeighborhoodInitializationRequest* messages, to which it replies
with corresponding *StartNeighborhoodInitializationResponse*.

The simulated proximity server sends *FinishNeighborhoodInitializationRequest* to the target proximity server and waits for *FinishNeighborhoodInitializationResponse* message.
...
The test sends *CreateActivityRequest* to create activity with location at [1, 2]:
  ...
  * `CreateActivityRequest.ignoreServerIds := []`
  * `ConversationRequest.signature` is a valid signature of `CreateActivityRequest.activity` part of the message using the test's identity private key
  * $localActivity := `CreateActivityRequest.activity`
...
The test then connects to neighbor port of the proximity server and on behalf of Server2 it verifies its identity and sends *NeighborhoodSharedActivityUpdateRequest*:
  ...
  [
    {
      ActionType = add, add = 
      {
        signedActivity.activity := 
        {
          version = [1,0,0],
          id = 1,
          ownerPublicKey is public key of Server2,
          profileServerContact.networkId = SHA256("test2"),
          profileServerContact.ipAddress = 127.0.0.1,
          profileServerContact.primaryPort = 16987,
          type = "Remote Activity",
          latitude = 10000000,
          longitude = 21000000,
          precision = 100,
          startTime is set to current UTC time stamp,
          expirationTime is set to current UTC time stamp + 1 hour,
          extraData = "a=b"
        }
        signedActivity.signature is a valid signature of signedActivity.activity
      }
    }
  ]`; $remoteActivity := `NeighborhoodSharedActivityUpdateRequest.items[0].add.signedActivity.activity`

and reads the response and closes this connection.
...
The test sends *GetActivityInformationRequest*:
  ...
  * `SingleRequest.version := [1,0,0]`
  * `GetActivityInformationRequest.id := 1`
  * `GetActivityInformationRequest.ownerNetworkId` is set to test identity public key
...
The test sends *GetActivityInformationRequest*:
  ...
  * `SingleRequest.version := [1,0,0]`
  * `GetActivityInformationRequest.id := 1`
  * `GetActivityInformationRequest.ownerNetworkId` is set to Server2 public key
...
Server replies with *NeighborhoodSharedActivityUpdateResponse*:
...
Server replies with *GetActivityInformationResponse*:
  ...
  * `Response.status == STATUS_OK`
  * `GetActivityInformationResponse.info.isPrimary == true`
  * `GetActivityInformationResponse.info.signedActivity.activity == $localActivity`
  * `GetActivityInformationResponse.info.signedActivity.signature` is a valid signature of `GetActivityInformationResponse.info.signedActivity.activity`
...
Server replies with *GetActivityInformationResponse*:
  ...
  * `Message.id == 5`
  * `Response.status == STATUS_OK`
  * `GetActivityInformationResponse.info.isPrimary == false`
  * `GetActivityInformationResponse.info.primaryServer.networkId == $Server2Id`
  * `GetActivityInformationResponse.info.primaryServer.ipAddress == $ServerIP`
  * `GetActivityInformationResponse.info.primaryServer.primaryPort == $BasePort2`
  * `GetActivityInformationResponse.info.signedActivity.activity == $remoteActivity`
  * `GetActivityInformationResponse.info.signedActivity.signature` is a valid signature of `GetActivityInformationResponse.info.signedActivity.activity`
