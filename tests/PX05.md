...
The test generates 50,050 activities. Activities number 49,991 to 50,050 will have type set to "last".
...
The test simulates a proximity server instance and establishes a neighborhood relationship with the target proximity server 
by invoking the neighborhood initialization process. Before it completes the process, it creates new activities. 
The initialization process then fails to complete and the test tries it again. Then the test creates more activities. 
The test's simulated proximity server must receive all activities from the target server.
...
The test creates a primary identity which it uses to simulate another proximity server and then a secondary activity which 
it uses for creation of new activities on the target proximity server. Then it creates 10 test activities.
...
The test then connects to proximity server's neighbor port and verifies its activity.
...
The test creates 5 more activities on the target server. 
...
The simulated proximity server received neighborhood updates in form of *NeighborhoodSharedActivityUpdateRequest* 
and together with the initialization process it received information about all the test activities.
...
The test creates 5 more activities on the target proximity server.
...
The test checks the activities received by its first client.
...
The target proximity server sends *NeighborhoodSharedActivityUpdateRequest* with updates containing information about the newly created activities.
...
The test simulates a proximity server instance and establishes a neighborhood relationship with the target proximity server 
by invoking the neighborhood initialization process. The test sets up new activities on the target server in order to 
receive neighborhood updates from it and it updates and deletes on of the activities quickly. At the same time it 
...
The test creates a primary identity which it uses to simulate another proximity server and then 
it creates 5 test activities on the target server.
...
The test creates 10 more activities on the target server. Then it selects one of the 15 activities it created and changes its extra data information.
Subsequently it deletes this activity. Then it waits 20 seconds.
...
The test successfully changes the extra data information of the activity and then it successfully 
deletes it from the target proximity server.
...
#### PX05028 - Neighborhood Updates - No Propagation
...
The test simulates a proximity server instance and establishes a neighborhood relationship with the target proximity server 
by invoking the neighborhood initialization process. The test sets up activities on the target server and manipulates them 
in order to receive neighborhood updates from it. Some of the updates are sent with no propagation flag set or changing 
only values that will not cause propagation.
...
The test selects 35 existing activities and changes some of them in a way it invokes the neighborhood propagation on the proximity server
and some of them in a way it will not. Then it waits 20 seconds.
...
The simulated proximity server receives neighborhood updates in form of *NeighborhoodSharedActivityUpdateRequest* 
for all the updated activities on the target proximity server which changes should be propagated.
