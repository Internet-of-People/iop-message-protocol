...
Server replies with *ListRolesResponse*:
  ...
  * Intersection of the set of port numbers of *primary* role and the set of port numbers of *srNeighbor*, *clNonCustomer*, *clCustomer*, and *clAppService* roles must be empty (i.e. no client only role is served on the same port as a profile server role; this also means that no port is used for both TLS and non-TLS service).
...
The test connects to the primary port of the profile server and sends *GetIdentityInformationRequest*:
